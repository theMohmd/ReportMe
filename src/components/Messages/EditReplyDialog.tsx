...
import { apiPatchMessageRepliesInputType } from "src/api/messages/message-replies/apiPatchMessageReplies";
import { usePatchMessageReply } from "./hooks/usePatchMessageReply";
...
const EditReplyDialog = ({
    ...
    //patch reply
    const { mutate } = usePatchMessageReply();
    ...
    const onSubmit: SubmitHandler<FormFields> = async (formData) => {
        //create input data
        const newData: apiPatchMessageRepliesInputType = {
            content:undefined,
            message_reply: data.id,
            file: undefined,
        };
    ...
    return (
        <Dialog
            close={close}
            title={t("Messages.edit", { what: t("Messages.reply") })}
        >
            <form
            ...
            >
                <Textarea
                    ...
                    placeholder={t("Messages.reply") + ": " + data.content}
                    {...register("content")}
                />
                ...
                <div className="flex flex-col gap-2 md:flex-row">
                    ...
                    ******************************************************************************/}
                    <button
                        ...
                        disabled={isSubmitting}
                    >
                        {isSubmitting ? <Loader /> : <>{t("Messages.submit")}</>}
                    </button>
                </div>
            </form>
        </Dialog>
